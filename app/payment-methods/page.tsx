    ...
    <div className="min-h-screen bg-[#0f172a] text-white">
      ...
      <main className="pt-20 pb-24 px-4 max-w-2xl mx-auto">
            ...
            <button
              ...
              className={`w-full p-4 rounded-lg flex justify-between items-center transition-all
                ${selectedMethod === method.id
                  ...
                  : "bg-[#2a2746] hover:bg-[#37335e]"}`
              }
            >
              <span>{method.name}</span>
              <img src={method.icon} alt={method.name} className="w-12 h-8" />
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mt-8">
          <button
            onClick={() => router.back()}
            className="py-3 bg-red-500 text-white rounded-lg hover:bg-red-600 transition"
          >
            Hủy
          </button>
          <button
        ...
        {showSuccessPopup && (
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
            <div className="bg-[#1d1b35] border border-[#37335e] rounded-xl p-6 max-w-md w-full mx-4 text-center">
              <h2 className="text-2xl font-bold mb-4 text-white">🎉 Thành công!</h2>
              <p className="text-gray-300 mb-6">Cảm ơn bạn đã sử dụng dịch vụ!</p>

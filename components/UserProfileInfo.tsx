  ...
  useEffect(() => {
    ...
    if (user) {
      setUserInfo({
        ...
        password: "", // Giữ mật khẩu rỗng để người dùng nhập lại
      });
    }
  }, []);
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (contentType && contentType.includes("application/json")) {
        ...
        // Nếu không phải JSON, lấy phản hồi dưới dạng văn bản và hiển thị như thông báo thành công
        const text = await response.text();
        alert(`Thông báo: ${text}`); // Hiển thị thông báo thành công từ server
        console.log("Phản hồi từ server:", text);  // In ra để kiểm tra phản hồi thực tế
      }
    } else {
